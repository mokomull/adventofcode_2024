    ...
    pub fn part2(&self) -> anyhow::Result<String> {
        ...
        for &(i, j) in &self.bytes {
            ...
            if last_path.is_none() || last_path.as_ref().unwrap().contains(&(i, j)) {
                match dfs_end::<DIMENSION>(&corrupted) {
...
fn dfs_end<const DIMENSION: u8>(corrupted: &HashSet<(u8, u8)>) -> anyhow::Result<Vec<(u8, u8)>> {
    let mut to_visit = vec![((0, 0), vec![])];
    let mut visited = HashSet::new();

    while let Some(((i, j), mut p)) = to_visit.pop() {
        if !visited.insert((i, j)) {
            // we've already been here
            continue;
        }
        ...
        if i > 0 {
            to_visit.push(((i - 1, j), p.clone()));
        }
        if j > 0 {
            to_visit.push(((i, j - 1), p.clone()));
        }
        if i < DIMENSION {
            to_visit.push(((i + 1, j), p.clone()));
        }
        if j < DIMENSION {
            to_visit.push(((i, j + 1), p));
        }
    }
